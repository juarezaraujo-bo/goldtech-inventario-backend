...
exports.collect = (req, res) => {
  ...
  db.get("SELECT id FROM clients WHERE name = ?", [clientName], (err, clientRow) => {
    if (err) return res.status(500).json({ message: err.message });

    if (!clientRow) {
      db.run("INSERT INTO clients (name, status) VALUES (?, 'Ativo')", [clientName], function (err) {
    ...
    if (equip) {
      ...
      const updateQuery = `
        UPDATE equipments SET 
          ...
          ip = ?, mac = ?, dominio = ?, antivirus = ?, ultima_inicializacao = ?,
          ultima_coleta = ?, origem_cadastro = 'agente'
        WHERE id = ?
      `;
      db.run(updateQuery, [
