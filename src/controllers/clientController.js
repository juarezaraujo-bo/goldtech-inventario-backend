...
exports.create = (req, res) => {
  ...
  db.run(query, [name, cnpj, contact_person, phone, email, address, observations, status || 'Ativo'], function(err) {
    if (err) return res.status(500).json({ message: err.message });
...
exports.update = (req, res) => {
  ...
  db.run(query, [name, cnpj, contact_person, phone, email, address, observations, status, id], function(err) {
    if (err) return res.status(500).json({ message: err.message });
...
exports.delete = (req, res) => {
  const { id } = req.params;
  db.get('SELECT COUNT(*) as count FROM equipments WHERE client_id = ?', [id], (err, row) => {
    if (err) return res.status(500).json({ success: false, message: err.message });
    if (row.count > 0) {
      return res.status(400).json({
        success: false,
        message: `Este cliente possui ${row.count} equipamento(s) vinculado(s). Desative o cliente ou mova os equipamentos antes de excluir.`
      });
    }
